...
GENERATE_MODEL = "gemini-2.0-flash"  # 1500 req/day free tier, works with v1beta API
EMBED_MODEL = "gemini-embedding-001"


def _clean_error(e: Exception) -> str:
    """Extract a short, readable message from a Gemini API exception."""
    msg = str(e)
    # Try to extract just the human-readable message field from the JSON blob
    import re as _re
    match = _re.search(r"'message':\s*'([^']+)'", msg)
    if match:
        return match.group(1).split("\\n")[0]  # first line only
    # Fallback: strip to first 120 chars
