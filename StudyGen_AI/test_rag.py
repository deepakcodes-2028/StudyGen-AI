...
def main():
    ...
    # Test local similarity search query
    print("\nTesting local similarity search query...")
    try:
        data = engine._load_doc(doc_id)
        query_embedding = engine._get_embeddings(["What is the vector dimension and embedding model used?"])[0]
        similarities = engine._cosine_similarity(query_embedding, data["embeddings"])
        best_idx = similarities.argmax()
        print("Local Query Success!")
        print(f"Retrieved Chunk: {data['chunks'][best_idx]}")
        print(f"Source Page: {data['metadatas'][best_idx]['page']}")
    except Exception as e:
        print(f"Local Query Failed: {str(e)}")
